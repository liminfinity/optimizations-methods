...
def tangent(
    ...
    """
    Находит минимум функции с помощью метода касательных.
    ...
    x = (b * d_fn(b) - a * d_fn(a) + fn(a) - fn(b)) / (d_fn(b) - d_fn(a))
    d_y = d_fn(x)
    while abs(d_y) > eps:
        if d_fn(x) > 0:
            b = x
...
if __name__ == "__main__":
    ...
    a, b = -0.5, 1.5
    eps = 10**-3
    res = tangent(fn, d_first_fn, a, b, eps)
